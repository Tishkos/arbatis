...
interface PrintMotorcyclesDialogProps {
  ...
  currentPageMotorcycles: Motorcycle[]
  currentPageCount: number
  totalCount: number
}

export function PrintMotorcyclesDialog({
  open,
  onOpenChange,
  currentPageMotorcycles,
  currentPageCount,
  totalCount,
}: PrintMotorcyclesDialogProps) {
  const params = useParams()
  const locale = (params?.locale as string) || 'ku'
  const t = useTranslations('motorcycles.print')
  ...
  const ALL_COLUMNS = React.useMemo(() => [
    ...
    { id: 'brand', label: t('columnLabels.brand') },
    { id: 'model', label: t('columnLabels.model') },
    { id: 'sku', label: t('columnLabels.sku') },
    { id: 'year', label: t('columnLabels.year') },
    { id: 'engineSize', label: t('columnLabels.engineSize') },
    { id: 'vin', label: t('columnLabels.vin') },
    { id: 'color', label: t('columnLabels.color') },
    { id: 'usdRetailPrice', label: t('columnLabels.retailPrice') },
    { id: 'usdWholesalePrice', label: t('columnLabels.wholesalePrice') },
  ...
  // Get motorcycles for preview
  const previewMotorcycles = React.useMemo(() => {
    return printScope === 'current' ? currentPageMotorcycles : []
  }, [printScope, currentPageMotorcycles])
  ...
  const handlePrint = async () => {
    ...
    try {
      let motorcyclesToPrint = currentPageMotorcycles

      if (printScope === 'all') {
        // Fetch all motorcycles
        const response = await fetch('/api/motorcycles?pageSize=10000')
        const data = await response.json()
        motorcyclesToPrint = data.motorcycles || []
      }
              ...
              <RadioGroup value={printScope} onValueChange={(value: 'current' | 'all') => setPrintScope(value)}>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <Label htmlFor="all" className={cn("font-normal cursor-pointer", fontClass)}>
                    {t('allMotorcycles', { count: totalCount })}
                  </Label>
                </div>
              </RadioGroup>
            </div>
              ...
              <div className="space-y-2">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse" style={{ fontSize: `${fontSize}pt` }}>
                    ...
                    <tbody>
                      {previewMotorcycles.slice(0, 10).map((motorcycle, idx) => (
                        <tr key={motorcycle.id || idx} className="border-b">
                            ...
                            <td className="border" style={{ padding: `${cellPadding * 2}px`, width: `${(imageSize + imagePadding * 2) * 3.78}px` }}>
                              ...
                              <div 
                                ...
                                style={{ 
                                  width: `${imageSize * 3.78}px`, 
                                  height: `${imageSize * 3.78}px`,
                                  display: motorcycle.image ? 'none' : 'block',
                                  margin: '0 auto'
                                }}
                              ></div>
                            </td>
                          ...
                          {selectedColumns.includes('id') && (
                            <td className={cn("border font-mono", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>
                              {motorcycle.id.slice(0, 8)}...
                            </td>
                          )}
                          {selectedColumns.includes('brand') && (
                            <td className={cn("border", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>{motorcycle.brand}</td>
                          )}
                          {selectedColumns.includes('model') && (
                            <td className={cn("border", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>{motorcycle.model}</td>
                          )}
                          {selectedColumns.includes('sku') && (
                            <td className={cn("border font-mono", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>{motorcycle.sku}</td>
                          )}
                          {selectedColumns.includes('year') && (
                            <td className={cn("border", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>{motorcycle.year || '-'}</td>
                          )}
                          {selectedColumns.includes('engineSize') && (
                            <td className={cn("border", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>{motorcycle.engineSize || '-'}</td>
                          )}
                          {selectedColumns.includes('vin') && (
                            <td className={cn("border font-mono", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>{motorcycle.vin || '-'}</td>
                          )}
                          {selectedColumns.includes('color') && (
                            <td className={cn("border", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>{motorcycle.color || '-'}</td>
                          )}
                          {selectedColumns.includes('usdRetailPrice') && (
                            <td className={cn("border text-right", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>
                              ${typeof motorcycle.usdRetailPrice === 'number' 
                                ? motorcycle.usdRetailPrice.toLocaleString() 
                                : parseFloat(motorcycle.usdRetailPrice).toLocaleString()}
                            </td>
                          )}
                          {selectedColumns.includes('usdWholesalePrice') && (
                            <td className={cn("border text-right", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>
                              ${typeof motorcycle.usdWholesalePrice === 'number' 
                                ? motorcycle.usdWholesalePrice.toLocaleString() 
                                : parseFloat(motorcycle.usdWholesalePrice).toLocaleString()}
                            </td>
                          )}
                            ...
                            <td className={cn("border text-right", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>
                              {motorcycle.rmbPrice 
                                ? (typeof motorcycle.rmbPrice === 'number' 
                                  ? motorcycle.rmbPrice.toLocaleString() 
                                  : parseFloat(motorcycle.rmbPrice).toLocaleString())
                                : '-'}
                            </td>
                          )}
                          ...
                          {selectedColumns.includes('lowStockThreshold') && (
                            <td className={cn("border text-right", fontClass)} style={{ padding: `${cellPadding * 2}px` }}>
                              {motorcycle.lowStockThreshold}
                            </td>
                          )}
                        </tr>
                      ))}
                      {previewMotorcycles.length === 0 && (
                        <tr>
                          <td 
                            colSpan={selectedColumns.length} 
                            className={cn("border p-4 text-center text-muted-foreground", fontClass)}
                          >
                            {t('noMotorcyclesPreview')}
                          </td>
                        </tr>
                      )}
                      {previewMotorcycles.length > 10 && (
                        <tr>
                          <td 
                            colSpan={selectedColumns.length} 
                            className={cn("border p-2 text-center text-xs text-muted-foreground", fontClass)}
                          >
                            {t('andMore', { count: previewMotorcycles.length - 10 })}
                          </td>
                        </tr>
                      )}
                    </tbody>
...
async function generateHTML(
  ...
  const getTranslationKey = (colId: string): string => {
    const keyMap: Record<string, string> = {
      ...
      'usdWholesalePrice': 'wholesalePrice',
    }
    const mappedKey = keyMap[colId] || colId
    return `motorcycles.print.columnLabels.${mappedKey}`
  }
  ...
  // Build rows
  const rows = motorcycles.map(motorcycle => {
    const row: any[] = []
    ...
    if (selectedColumns.includes('vin')) row.push(motorcycle.vin || '-')
    if (selectedColumns.includes('color')) row.push(motorcycle.color || '-')
    if (selectedColumns.includes('usdRetailPrice')) {
      const price = typeof motorcycle.usdRetailPrice === 'number' ? motorcycle.usdRetailPrice : parseFloat(String(motorcycle.usdRetailPrice))
      row.push(Math.round(price).toLocaleString('en-US') + ' ع.د')
    }
    if (selectedColumns.includes('usdWholesalePrice')) {
      const price = typeof motorcycle.usdWholesalePrice === 'number' ? motorcycle.usdWholesalePrice : parseFloat(String(motorcycle.usdWholesalePrice))
      row.push(Math.round(price).toLocaleString('en-US') + ' ع.د')
    }
    if (selectedColumns.includes('rmbPrice')) {
      const price = motorcycle.rmbPrice ? (typeof motorcycle.rmbPrice === 'number' ? motorcycle.rmbPrice : parseFloat(String(motorcycle.rmbPrice))) : 0
      row.push(price ? Math.round(price).toLocaleString('en-US') + ' ع.د' : '-')
    }
    if (selectedColumns.includes('stockQuantity')) row.push(motorcycle.stockQuantity)
    if (selectedColumns.includes('lowStockThreshold')) row.push(motorcycle.lowStockThreshold)
    return row
  })
...
<head>
  ...
  <title>${escapeHtml(translate('motorcycles.print.pdfContent.motorcyclesList'))}</title>
  <style>
    @page {
      size: A4 ${orientation};
      margin: 1cm;
...
<body>
  <div class="print-container">
    ${headerType === 'logo' ? `
      <div class="header">
        <h1>${escapeHtml(translate('motorcycles.print.pdfContent.motorcyclesList'))}</h1>
        <div class="date">${escapeHtml(translate('motorcycles.print.pdfContent.generated'))}: ${escapeHtml(customDate || new Date().toLocaleDateString(pdfLanguage === 'ku' ? 'ku' : pdfLanguage === 'ar' ? 'ar' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }))}</div>
      </div>
    ` : headerType === 'date' ? `
      <div class="header">
        <div class="date">${escapeHtml(translate('motorcycles.print.pdfContent.generated'))}: ${escapeHtml(customDate || new Date().toLocaleDateString(pdfLanguage === 'ku' ? 'ku' : pdfLanguage === 'ar' ? 'ar' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' }))}</div>
    ...
    ` : headerType === 'custom' && customHeaderImage ? `
      <div class="header">
        <img src="${getAbsoluteImageUrl(customHeaderImage)}" alt="Header" style="max-width: 100%; max-height: 40mm; width: auto; height: auto; display: block; margin: 0 auto;" onerror="console.error('Failed to load header image'); this.style.display='none';" />
      </div>
    ` : ''}
    ...
    ${showFooter || showPageNumbers ? `
      <div class="footer">
        ${showPageNumbers ? `<div>${escapeHtml(translate('motorcycles.print.pdfContent.page'))} <span class="page-number">1</span> ${escapeHtml(translate('motorcycles.print.pdfContent.of'))} <span class="total-pages">1</span></div>` : ''}
...
async function generatePDF(
  ...
  if (headerType === 'logo') {
    ...
    await renderText(
      t('motorcycles.print.pdfContent.motorcyclesList'),
      pageWidth / 2,
      headerHeight / 2 + 3,
      {
        fontSize: 24,
    ...
    await renderText(
      `${t('motorcycles.print.pdfContent.generated')}: ${dateStr}`,
      margin,
      margin + 5,
      {
        fontSize: 10,
    ...
    await renderText(
      `${t('motorcycles.print.pdfContent.generated')}: ${dateText}`,
      margin,
      startY,
      {
        fontSize: 10,
  ...
  const headers = allColumns
    ...
    .map(col => {
      ...
      if (col.id === 'usdRetailPrice') columnKey = 'retailPrice'
      else if (col.id === 'usdWholesalePrice') columnKey = 'wholesalePrice'
      else if (col.id === 'stockQuantity') columnKey = 'quantity'
      return t(`motorcycles.print.columnLabels.${columnKey}`) || col.label
    })
  
  // Load all images first
  const imageMap = new Map<string, string | null>()
  if (selectedColumns.includes('image')) {
    const imagePromises = motorcycles
      .filter(m => m.image)
      .map(async (motorcycle) => {
        try {
          const response = await fetch(motorcycle.image!)
          const blob = await response.blob()
          const imgData = await new Promise<string>((resolve, reject) => {
            const reader = new FileReader()
            reader.onloadend = () => resolve(reader.result as string)
          ...
          return { motorcycleId: motorcycle.id, imgData }
        } catch (error) {
          console.warn(`Could not load image for motorcycle ${motorcycle.id}:`, error)
          return { motorcycleId: motorcycle.id, imgData: null }
        }
      })

    const imageResults = await Promise.all(imagePromises)
    imageResults.forEach(({ motorcycleId, imgData }) => {
      imageMap.set(motorcycleId, imgData)
    })
  }
  ...
  const rows = motorcycles.map((motorcycle) => {
    ...
    if (selectedColumns.includes('vin')) row.push(motorcycle.vin || 'N/A')
    if (selectedColumns.includes('color')) row.push(motorcycle.color || 'N/A')
    if (selectedColumns.includes('usdRetailPrice')) {
      const price = typeof motorcycle.usdRetailPrice === 'number' ? motorcycle.usdRetailPrice : parseFloat(motorcycle.usdRetailPrice)
      row.push(Math.round(price).toLocaleString('en-US') + ' ع.د')
    }
    if (selectedColumns.includes('usdWholesalePrice')) {
      const price = typeof motorcycle.usdWholesalePrice === 'number' ? motorcycle.usdWholesalePrice : parseFloat(motorcycle.usdWholesalePrice)
      row.push(Math.round(price).toLocaleString('en-US') + ' ع.د')
    }
    if (selectedColumns.includes('rmbPrice')) {
      const price = motorcycle.rmbPrice ? (typeof motorcycle.rmbPrice === 'number' ? motorcycle.rmbPrice : parseFloat(motorcycle.rmbPrice)) : 0
      row.push(price ? Math.round(price).toLocaleString('en-US') + ' ع.د' : '-')
    }
    if (selectedColumns.includes('stockQuantity')) row.push(motorcycle.stockQuantity)
    if (selectedColumns.includes('lowStockThreshold')) row.push(motorcycle.lowStockThreshold)
  ...
  autoTable(doc, {
    ...
    didDrawCell: (data: any) => {
      ...
      if (hasImageColumn && data.column.index === imageColumnIndex && data.section === 'body') {
        ...
        const motorcycle = motorcycles[rowIndex]
        if (motorcycle && motorcycle.image) {
          const imgData = imageMap.get(motorcycle.id)
          if (imgData) {
            try {
              ...
              console.warn(`Could not add image for motorcycle ${motorcycle.id}:`, error)
            }
          } else {
    ...
    didDrawPage: (data: any) => {
      ...
      if (showFooter || showPageNumbers) {
        ...
        if (showPageNumbers) {
          const pageText = `${t('motorcycles.print.pdfContent.page')} ${pageNumber} ${t('motorcycles.print.pdfContent.of')} ${totalPages}`
  ...
  doc.save(`motorcycles-${new Date().getTime()}.pdf`)
}

async function generateXLSX(motorcycles: Motorcycle[], selectedColumns: string[], allColumns: Array<{ id: string; label: string }> = []) {
  ...
  const rows = motorcycles.map(motorcycle => {
    ...
    if (selectedColumns.includes('id')) row['ID'] = motorcycle.id.slice(0, 8)
    if (selectedColumns.includes('brand')) row['Brand'] = motorcycle.brand
    if (selectedColumns.includes('model')) row['Model'] = motorcycle.model
    if (selectedColumns.includes('sku')) row['SKU'] = motorcycle.sku
    if (selectedColumns.includes('year')) row['Year'] = motorcycle.year || ''
    if (selectedColumns.includes('engineSize')) row['Engine Size'] = motorcycle.engineSize || ''
    if (selectedColumns.includes('vin')) row['VIN'] = motorcycle.vin || ''
    if (selectedColumns.includes('color')) row['Color'] = motorcycle.color || ''
    if (selectedColumns.includes('usdRetailPrice')) {
      const price = typeof motorcycle.usdRetailPrice === 'number' ? motorcycle.usdRetailPrice : parseFloat(motorcycle.usdRetailPrice)
      row['Retail Price (USD)'] = price
    }
    if (selectedColumns.includes('usdWholesalePrice')) {
      const price = typeof motorcycle.usdWholesalePrice === 'number' ? motorcycle.usdWholesalePrice : parseFloat(motorcycle.usdWholesalePrice)
      row['Wholesale Price (USD)'] = price
    }
    if (selectedColumns.includes('rmbPrice')) {
      const price = motorcycle.rmbPrice ? (typeof motorcycle.rmbPrice === 'number' ? motorcycle.rmbPrice : parseFloat(motorcycle.rmbPrice)) : null
      row['RMB Price (¥)'] = price || ''
    }
    if (selectedColumns.includes('stockQuantity')) row['Quantity'] = motorcycle.stockQuantity
    if (selectedColumns.includes('lowStockThreshold')) row['Low Stock Threshold'] = motorcycle.lowStockThreshold
    return row
  })
  ...
  XLSX.writeFile(workbook, `motorcycles-${new Date().getTime()}.xlsx`)
}
