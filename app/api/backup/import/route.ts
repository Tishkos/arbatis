        ...
        const motorcycles = await Promise.all(
          data.data.motorcycles.map(async (motorcycle: any) => {
            ...
            return prisma.motorcycle.upsert({
              ...
              update: {
                // Use name field (brand/model/year/etc. have been replaced with name)
                name: motorcycle.name || motorcycle.brand || `${motorcycle.brand || ''} ${motorcycle.model || ''}`.trim() || 'Motorcycle',
                sku: motorcycle.sku,
                image: imageUrl,
                attachment: attachments,
